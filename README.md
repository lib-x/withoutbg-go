...
cutout, err := r.Remove(img)  // *image.RGBA：原尺寸，alpha 就是蒙版
...
- **两步输出**：`Alpha` 给蒙版，`Remove` 给带 alpha 通道的抠图，都是原图尺寸
...
`go test ./...`：单元测试覆盖 sidecar 解析、letterbox 几何、输入张量取值与黑底填充、蒙版裁剪与缩放、alpha 量化、合成；golden 测试拿官方示例三联图（输入 / 抠图 / 蒙版）做对比，三个样本的前景 IoU 分别是 0.869、0.973、0.991，平均绝对误差 0.038、0.016、0.082（示例图是缩放过的发布件，边缘像素不会逐点一致，因此断言留了余量）。模型缺失时相关测试自动跳过。
